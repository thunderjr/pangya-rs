...
/// Tutorial families defined by the U.S. 852 client.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum TutorialKind {
    /// Rookie missions, represented by flags 1 through 128.
    Rookie,
    /// Beginner missions, represented by flags 256 through 8192.
    Beginner,
}

/// Durable U.S. 852 tutorial masks. A mask is monotonic and therefore safe to replay.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct TutorialProgress {
    /// Completed Rookie mission flags.
    pub rookie: u32,
    /// Completed Beginner mission flags.
    pub beginner: u32,
}

/// All source-defined Rookie mission flags.
pub const TUTORIAL_ROOKIE_MASK: u32 = 0xff;
/// All source-defined Beginner mission flags.
pub const TUTORIAL_BEGINNER_MASK: u32 = 0x3f00;

impl TutorialProgress {
    /// Applies one source-defined mission flag idempotently.
    #[must_use]
    pub const fn apply(self, kind: TutorialKind, mission: u32) -> Option<Self> {
        let valid = match kind {
            TutorialKind::Rookie => {
                mission != 0 && mission <= TUTORIAL_ROOKIE_MASK && mission.is_power_of_two()
            }
            TutorialKind::Beginner => {
                mission >= 0x100 && mission <= 0x2000 && mission.is_power_of_two()
            }
        };
        if !valid {
            return None;
        ...
        Some(match kind {
            TutorialKind::Rookie => Self {
                rookie: self.rookie | mission,
                ..self
            },
            TutorialKind::Beginner => Self {
                beginner: self.beginner | mission,
                ..self
            },
        })
    }

    /// Applies one mission and reports a completion transition.
    #[must_use]
    pub const fn apply_result(
        self,
        kind: TutorialKind,
        mission: u32,
    ) -> Option<TutorialMissionResult> {
        let Some(next) = self.apply(kind, mission) else {
            return None;
        };
        let newly_completed = match kind {
            TutorialKind::Rookie => self.rookie & mission == 0,
            TutorialKind::Beginner => self.beginner & mission == 0,
        };
        let completed_option = match kind {
            TutorialKind::Rookie if !self.rookie_complete() && next.rookie_complete() => Some(1),
            TutorialKind::Beginner if !self.beginner_complete() && next.beginner_complete() => {
                Some(2)
            }
            _ => None,
        };
        Some(TutorialMissionResult {
            newly_completed,
            progress: next,
            completed_option,
        })
    ...
    /// Whether the Rookie sequence is complete.
    #[must_use]
    pub const fn rookie_complete(self) -> bool {
        self.rookie & TUTORIAL_ROOKIE_MASK == TUTORIAL_ROOKIE_MASK
    }

    /// Whether the Beginner sequence is complete.
    #[must_use]
    pub const fn beginner_complete(self) -> bool {
        self.beginner & TUTORIAL_BEGINNER_MASK == TUTORIAL_BEGINNER_MASK
    }
}

/// Exact completion reward item from K4T Py_Source_US `MailSender.AddItemTutorialEvent`.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// Result of applying one tutorial mission to durable progression.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct TutorialMissionResult {
    /// Whether the mission flag was newly observed.
    pub newly_completed: bool,
    /// Durable masks after the operation.
    pub progress: TutorialProgress,
    /// Completion option newly reached, if any (`1` Rookie, `2` Beginner).
    pub completed_option: Option<u8>,
}

/// Returns the exact per-mission reward from K4T Py_Source_US `MailSender.AddItemTutorial`.
#[must_use]
pub const fn tutorial_mission_reward(kind: TutorialKind, mission: u32) -> Option<TutorialReward> {
    let reward = match (kind, mission) {
        (TutorialKind::Rookie, 1) => (0x1a00_000f, 3),
        (TutorialKind::Rookie, 2) => (0x1800_0007, 3),
        (TutorialKind::Rookie, 4) => (0x1800_0005, 3),
        (TutorialKind::Rookie, 8) => (0x1800_0008, 3),
        (TutorialKind::Rookie, 16) => (0x1a00_0010, 500),
        (TutorialKind::Rookie, 32) => (0x1800_0004, 3),
        (TutorialKind::Rookie, 64) => (0x1a00_0010, 500),
        (TutorialKind::Rookie, 128) => (0x1a00_0010, 1000),
        (TutorialKind::Beginner, 256) => (0x1a00_000f, 3),
        (TutorialKind::Beginner, 512) => (0x1800_0028, 1),
        (TutorialKind::Beginner, 1024) => (0x1800_0006, 1),
        (TutorialKind::Beginner, 2048) => (0x1800_0007, 5),
        (TutorialKind::Beginner, 4096) => (0x1800_0000, 4),
        (TutorialKind::Beginner, 8192) => (0x1800_0001, 4),
        _ => return None,
    };
    Some(TutorialReward {
        item_type_id: ItemTypeId::new(reward.0),
        quantity: reward.1,
    })
}

/// Returns the two-item completion reward for a completed tutorial family.
#[must_use]
pub const fn tutorial_completion_rewards(option: u8) -> Option<[TutorialReward; 2]> {
    match option {
        1 => Some([
            TutorialReward {
                item_type_id: ItemTypeId::new(0x1c00_0000),
                quantity: 1,
            },
            TutorialReward {
                item_type_id: ItemTypeId::new(0x1000_0012),
                quantity: 1,
            },
        ]),
        2 => Some([
            TutorialReward {
                item_type_id: ItemTypeId::new(0x1800_0027),
                quantity: 10,
            },
            TutorialReward {
                item_type_id: ItemTypeId::new(0x1a00_0010),
                quantity: 10_000,
            },
        ]),
        _ => None,
...
pub trait PlayerRepository: Send + Sync {
    ...
    /// Loads the durable U.S. 852 tutorial masks.
    fn load_tutorial_progress(
        &self,
        _account_id: AccountId,
    ) -> RepositoryFuture<'_, Result<TutorialProgress, RepositoryError>> {
        Box::pin(std::future::ready(Ok(TutorialProgress::default())))
    }

    /// Atomically applies one validated mission flag to the durable tutorial masks.
    ...
    /// Claims one completion option exactly once; reward insertion must share this transaction in
    /// concrete repositories so retries cannot duplicate catalog-resolved items.
    fn claim_tutorial_completion(
        &self,
        _account_id: AccountId,
        _option: u8,
        _rewards: [TutorialReward; 2],
    ) -> RepositoryFuture<'_, Result<bool, RepositoryError>> {
        Box::pin(std::future::ready(Err(RepositoryError::Storage(
            StorageFault::Other,
        ))))
...
mod tests {
    ...
    #[test]
    fn tutorial_masks_are_monotonic_and_complete_only_at_reference_masks() {
        let mut progress = TutorialProgress::default();
        for mission in [1, 2, 4, 8, 16, 32, 64, 128] {
            progress = progress
                .apply(TutorialKind::Rookie, mission)
                .unwrap_or_else(|| unreachable!());
        }
        assert!(progress.rookie_complete());
        assert_eq!(progress.apply(TutorialKind::Rookie, 128), Some(progress));
        assert_eq!(progress.apply(TutorialKind::Rookie, 3), None);
        assert!(!progress.beginner_complete());
        let result = TutorialProgress {
            rookie: 0x7f,
            beginner: 0,
        }
        .apply_result(TutorialKind::Rookie, 128)
        .unwrap_or_else(|| unreachable!());
        assert_eq!(result.completed_option, Some(1));
        assert!(result.newly_completed);
    }

    #[test]
    fn tutorial_mission_rewards_match_us852_source() {
        let expected = [
            (TutorialKind::Rookie, 1, 0x1a00_000f, 3),
            (TutorialKind::Rookie, 2, 0x1800_0007, 3),
            (TutorialKind::Rookie, 4, 0x1800_0005, 3),
        ...
        for (kind, mission, item_type_id, quantity) in expected {
            assert_eq!(
                tutorial_mission_reward(kind, mission),
                Some(TutorialReward {
                    item_type_id: ItemTypeId::new(item_type_id),
                    quantity,
                })
            );
        }
    ...
    #[test]
    fn tutorial_completion_rewards_match_us852_source() {
        assert_eq!(
            tutorial_completion_rewards(1),
            Some([
                TutorialReward {
                    item_type_id: ItemTypeId::new(0x1c00_0000),
                    quantity: 1
