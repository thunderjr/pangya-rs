...
use pangya_domain::{
    AccountRepository, PlayerRepository, RepositoryError, TutorialKind, tutorial_completion_rewards,
};
...
fn account(username: &str) -> pangya_domain::NewAccount {
    pangya_domain::NewAccount {
        ...
        nickname: Some(pangya_domain::Nickname::parse("Tut").expect("nickname")),
...
async fn tutorial_progress_survives_restart_and_mission_retries(pool: PgPool) {
    ...
    let aggregate = repository
        .create_account(account("TutorialRestart"))
        .await
        .expect("account");
    ...
    for mission in [1, 2, 4, 8, 16, 32, 64] {
        let result = repository
            .apply_tutorial_mission(id, TutorialKind::Rookie, mission)
            .await
            .expect("mission");
    ...
    let complete = repository
        .apply_tutorial_mission(id, TutorialKind::Rookie, 128)
        .await
        .expect("final mission");
    ...
    assert_eq!(
        restarted.load_tutorial_progress(id).await.expect("restart"),
        complete.progress
    );
    let retry = restarted
        .apply_tutorial_mission(id, TutorialKind::Rookie, 128)
    ...
    let mission_rows: i64 =
        sqlx::query_scalar("SELECT count(*) FROM tutorial_mission_rewards WHERE account_id = $1")
            .bind(id.get())
            .fetch_one(&pool)
            .await
            .expect("mission ledger");
...
async fn tutorial_completion_claim_is_concurrent_and_exactly_once(pool: PgPool) {
    let repository = PgRepository::new(pool.clone());
    let aggregate = repository
        .create_account(account("TutorialConcurrent"))
        .await
        .expect("account");
    let id = aggregate.account.id;
    ...
    let claim_rows: i64 =
        sqlx::query_scalar("SELECT count(*) FROM tutorial_reward_claims WHERE account_id = $1")
            .bind(id.get())
            .fetch_one(&pool)
            .await
            .expect("claim rows");
...
async fn tutorial_completion_cannot_be_claimed_before_progress(pool: PgPool) {
    let repository = PgRepository::new(pool.clone());
    let aggregate = repository
        .create_account(account("TutorialNoPremature"))
        .await
        .expect("account");
    let rewards = tutorial_completion_rewards(1).expect("rookie rewards");
    assert_eq!(
        repository
            .claim_tutorial_completion(aggregate.account.id, 1, rewards)
            .await,
