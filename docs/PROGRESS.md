...
13. **Client-side startup crash before any socket** — narrowed substantially but still open. The client is protected with **WinLicense** (Oreans): attaching a debugger prints its banner and terminates the process, and the crash report's symbol names are meaningless because the image is packed. First-chance diagnosis therefore has to come from inside the process, which a vectored exception handler added to Rugburn provides. That identified the first failure exactly — `WAppException("Cannot open file.")` for `chat.bin`, then `nick.bin`, then `bbh.bin`, three files the client opens as loose files in its own directory rather than through its PAK filesystem. With those placed, the C++ exception is gone and the client gets further (its crash report now reaches the `Inst Dir` line). What remains is a deterministic access violation at `ProjectG.exe+0x488d08` reading `[ecx+0x30]` with `ecx` zero — a method called on a null `this`, with no filename anywhere on the stack, so not a missing file. Ruled out for this specific failure: theme JPEG decoding (an empty theme document reproduces it identically), loose files shadowing PAK content, the translation catalog's content, and hypervisor detection (the guest now reports `HypervisorPresent: False`). Getting further needs static analysis of an unpacked image, which is a different kind of work from everything above. Full detail and the handler's shape in [`evidence/REAL_CLIENT_STARTUP_2026-08-07.md`](evidence/REAL_CLIENT_STARTUP_2026-08-07.md).
