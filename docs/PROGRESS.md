...
28. **A real client crashes while loading a versus hole** — **open.** With the census and master fixes in place the client accepts Start, receives `0x0076`, `0x0052`, `0x009e` and `0x005b`, answers with `0x000c`, `0x0048` and `0x001a`, and then dies with an access violation reading `[null+4]` before it ever sends `0x0011`. One cause is fixed: the hole array in `0x0052` is a fixed `[18]HoleInfo` (`pangbox/server` `game/packet/server.go` `ServerRoomGameData`) that the client reads by width rather than by the `hole_count` beside it, so sending only the played holes left it parsing the seed and the collectible table as hole descriptors. That alone did not stop the crash. The client ships no usable symbols — every frame resolves to the one exported name — so the stack is not a lead, and the remaining difference has to come from the references rather than from guessing at the wire.
