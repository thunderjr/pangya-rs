...
24. **A room went stale while you sat in it, and a hole could not survive the client's own chatter** — **resolved 2026-08-08.** Two gaps found by reading what a real client sends around a versus hole rather than by driving one into it.
    ...
    Separately, thirteen in-match opcodes — aim rotation, the power meter, club changes, item use, relief drops, the client's own hole info, the active-player acknowledgement, pause, the aiming arrow, load progress, game end, last-player-leave — arrive during a hole and none of them changes a stroke, a turn, or a score. Under the shipped `unknown_opcode_policy = "disconnect"` each would have ended the session, so a hole could not have survived the first real shot. They are an explicit allowlist now, `RETAIL_ACCEPTED_MATCH_OPCODES`.

    The relay slice decodes checked, exact request bodies for aim (`0x0013`), power selection (`0x0015`), club (`0x0016`), visual item use (`0x0017`), comet relief (`0x0019`), and pause (`0x0030`), and delivers `0x0056`, `0x0058`, `0x0059`, `0x005a`, `0x0060`, and `0x008b` to the captured stroke roster with the authoritative sender connection ID. The protocol tests cover truncation, finite floats, fixed widths, and closed enums. Item use remains visual relay only: it is not claimed as inventory consumption.

    The remaining reference-defined compatibility inputs are now explicit: meter input (`0x0014`), hole metadata (`0x001a`), and active-user acknowledgement (`0x0022`) are exact-decoded and consumed in an active stroke; full-match statistics (`0x0006`) consumes the fixed 239-byte report and answers from the authenticated persisted snapshot, never client-claimed rewards; `0x0037` tears down finished-room membership; and lounge actions (`0x0063`) decode and relay as `0x00c4` only to current pre-match room occupants. The aiming arrow (`0x0042`) and game-end body remain deliberately opaque because references do not establish a stable client-safe response; they remain allowlisted rather than guessed. `0x0034` is the exception that needs an answer — the client waits to be told it may take the first shot, and `0x0090` carries nothing but its arrival.
