...
24. **A room went stale while you sat in it, and a hole could not survive the client's own chatter** — **resolved 2026-08-08.** Two gaps found by reading what a real client sends around a versus hole rather than by driving one into it.
    ...
    The first relay slice now decodes checked, exact request bodies for aim (`0x0013`), power selection (`0x0015`), club (`0x0016`), visual item use (`0x0017`), and comet relief (`0x0019`), and delivers `0x0056`, `0x0058`, `0x0059`, `0x005a`, and `0x0060` to the captured two-player stroke roster with the authoritative sender connection ID. The full retail TCP test has assertions for every projection and the protocol tests cover truncation and out-of-range closed enums. Item use is deliberately not claimed as consumption: it has no real retail consumable aggregate yet.

    `0x0014` remains accepted without a reply because PacketDoc's paired U.S.-profile definition explicitly says it has none, despite the issue's broad reference table associating meter input with `0x0058`; no lossless `f32` meter value fits the observed one-byte `0x0058` body. The bundled SuperSS snapshot does not contain the cited `packet_func_sv.h`, and the available PacketDoc snapshot lacks `0x0030` and `0x0042` definitions, so pause, aiming arrow, room teardown, client hole info, active-player acknowledgement, match-statistics submit, equipment sync, and lounge action remain explicitly open rather than guessed. `0x0034` is the exception that needs an answer — the client waits to be told it may take the first shot, and `0x0090` carries nothing but its arrival.
